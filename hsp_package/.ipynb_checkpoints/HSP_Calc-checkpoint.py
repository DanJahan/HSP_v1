...
filePath = r'hsp_packge/HSP_Costs.csv'
...
for i in range(len(rowsList)):
    sol = rowsList[i][1]
    solventList.append({'name': sol['Solvents'], 
                        'parameters' : np.array([sol['dD'], sol['dP'], sol['dH']]), 
                        'molvol' : sol['MVol'], 'cost' : sol['Cost'], 
...
#the solvents list
ParamCheck = (solParamMaxdD < Param[0]) or (solParamMindD > Param[0]) or \
             (solParamMaxdP < Param[1]) or (solParamMindP > Param[1]) or \
             (solParamMaxdH < Param[2]) or (solParamMindH > Param[2]) 
...
try:
    if TolParam < 0:
        print('Unknown parameter tolerance. Please enter a value greater than or equal to 0.')
except:
    print('Could not understand input. Please enter a number greater than or equal to 0.')
...
def solventblend(SolAmount, Sols):
    ...
    for i in range(len(Sols)): #for each solvent, multiply by the parameters by amount used
...
if runMode == 0:
    ans = minimize(Cost, np.ones(len(Sols))/len(Sols),
               constraints=[{"type" : "ineq", "fun": f} for f in [c1, c2]]+[{"type" : "eq", "fun" : Equal1}], 
               options={'maxiter' : 1000})

#runs minimize() on BlendDist()
elif runMode == 1:
    ans = minimize(BlendDist, np.ones(len(Sols))/len(Sols),
               constraints=[{"type" : "ineq", "fun": f} for f in [c1]]+[{"type" : "eq", "fun" : Equal1}], 
               options={'maxiter' : 1000})
...
limitAns = minimize(Cost, np.ones(len(Sols))/len(Sols),
               constraints=[{"type" : "ineq", "fun": f} for f in \
                            [c1, c2]]+[{"type" : "eq", "fun" : Equal1}], 
