...
def Create_Shelve(file_ext = "/HSP_v1/hsp_package/HSP_Costs.csv"):
  ...
  for i in range(len(rowsList)):
    ...
    solventList.append({
        ...
        sol["Solvents"],
        "parameters":
        np.array([sol["dD"], sol["dP"], sol["dH"]]),
        "molvol":
        sol["MVol"],
        "cost":
        sol["Cost"],
        "nfpa":
        np.array([sol["Health"], sol["Fire"], sol["Reactivity"]]),
        "stock":
        sol["Stock"],
    })
...
def solventblend(SolAmount, sols):
  ...
  for i in range(len(
      sols)):  # for each solvent, multiply by the parameters by amount used
...
def output(ans, hansen, sols):
  ...
  print(f"The cost of the solvent blend is ${np.round(Cost(rounded, sols), 3)} per liter.")